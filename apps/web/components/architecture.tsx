export function Architecture() {
  return (
    <section id="architecture" className="bg-surface-alt py-24 md:py-32">
      <div className="container-x">
        <div className="text-center mb-16">
          <p className="text-eyebrow uppercase tracking-widest text-accent">Architecture</p>
          <h2 className="mt-3 text-h2 text-balance">Thin layer. Strong contracts.</h2>
          <p className="mx-auto mt-5 max-w-2xl text-ink-muted">
            <code className="font-mono text-[14px]">@agentskit/os-core</code> stays under 15 KB
            gzipped. Everything else is independently installable. Use one piece without the desktop.
          </p>
        </div>

        <div className="mx-auto max-w-5xl rounded-3xl border border-line bg-white p-8 md:p-12 shadow-[0_30px_80px_-30px_rgba(0,0,0,0.12)]">
          <Layer
            title="OS Surfaces"
...
function Layer({
  ...
  const bg = tone === "ink" ? "bg-ink text-white" : tone === "accent" ? "bg-accent text-white" : "bg-surface-alt text-ink";
  if (wide) {
    return (
      <div className={`rounded-2xl ${bg} px-6 py-5 text-center`}>
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      {items.map((it) => (
        <div key={it} className={`rounded-xl ${bg} px-4 py-3 text-center font-mono text-[13px] font-medium`}>
          {it}
        </div>
      ))}
    </div>

...
function useSnapshotPanelState(): SnapshotPanelState {
    ...
    <GlassPanel className="flex w-full max-w-md flex-col overflow-hidden rounded-xl shadow-2xl">
      ...
      <div className="space-y-5 px-5 py-5">
        <p className="text-[13px] text-[var(--ag-ink-subtle)]">
          A snapshot bundles all persisted desktop state (preferences, shortcuts, theme, status line,
          notifications, focus mode, onboarding, and workspace selection) into a single JSON file.
          Export it to back up your configuration or share it across machines. Importing a snapshot
          re-applies all stored values and reloads the app.
        </p>
        ...
        <div className="space-y-2">
          ...
          />
          {importError && (
            <p role="alert" data-testid="import-error" className="text-[12px] text-[var(--ag-danger)]">
              {importError}
            </p>
          )}
          {importSuccess && (
            <p role="status" data-testid="import-success" className="text-[12px] text-[var(--ag-success)]">
...
export function SnapshotPanel({ isOpen, onClose }: SnapshotPanelProps) {
  const { fileInputRef, importError, importSuccess, handleExport, handleImportClick, handleFileChange } =
    useSnapshotPanelState()

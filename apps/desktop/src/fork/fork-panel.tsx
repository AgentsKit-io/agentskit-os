...
export function ForkPanel({
  ...
  const handleSave = async (): Promise<void> => {
    ...
    try {
      ...
      const res = await sidecarRequest<FlowCreateResponse>('flows.create', {
        draft: {
          ...draft,
      ...
      const flowId =
        res.flowId && res.flowId.length > 0
          ? res.flowId
          : `flow-${Date.now()}`
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-[var(--ag-line)] px-4 py-3">
          <div className="flex flex-col gap-0.5 min-w-0">
            ...
            <span
              className="text-xs font-mono text-[var(--ag-ink-muted)] truncate"
              title={traceId}
            >
              {traceId}
            </span>
          ...
          {savedFlowId !== null && (
            ...
            >
              ...
              <p className="mt-0.5 text-xs text-[var(--ag-ink-muted)]">
                New flow id:{' '}
                <span className="font-mono text-[var(--ag-ink)]">{savedFlowId}</span>
              </p>
              <button
                type="button"
                aria-label={`Go to flow ${savedFlowId}`}
          ...
          {savedFlowId === null && (
            <>
              ...
              <div className="flex flex-col gap-1">
                <label
                  htmlFor="fork-name"
                  className="text-xs font-medium text-[var(--ag-ink-muted)]"
                >
                  Flow name *
                </label>
                <input
              ...
              {draft.nodes.length > 0 && (
                <div className="flex flex-col gap-2">
                  <p className="text-xs font-medium text-[var(--ag-ink-muted)]">
                  ...
                  <div className="flex flex-col gap-2">
                    {draft.nodes.map((node, i) => (
                      <NodeRow
                        key={node.id}
                        node={node}
                        onChange={(updated) => handleNodeChange(i, updated)}
                      />
                    ))}
                  </div>
                </div>
